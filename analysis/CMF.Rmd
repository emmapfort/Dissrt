...
#this dataframe has Ensembl, Entrez, and Symbol
hc_cpm_mat <- readRDS("data/counts/hc_cpm_allids_df.RDS") %>% 
  as.matrix()
...
#I want to remove my outlier that I've had from PCAs - PFOA_24_H3
...
#also save as RData for TACC
# save(meta_cmf, file = "meta_cmf_reordered_all.RData")
...
# saveRDS(expr, "data/cmf/cormotif_reordered_cpm_matrix_all.RDS")
# save(expr, file = "cmf_cpm_reordered_all.RData")
expr <- readRDS("data/cmf/cormotif_reordered_cpm_matrix_all.RDS")
...
# write_csv(groupid_rmout, "data/cmf/groupid_noout.csv")

#now read in the one where I removed that outlier column
groupid <- read_csv("data/cmf/groupid_noout.csv")
...
#I haven't changed this one to have no outlier
...
#remove my outlier sample here
# groupid_24noout <- read_csv("data/cmf/groupid_24.csv", 
#     col_types = cols(PFOA_24_H3 = col_skip(), 
#         PFOA_24_C3 = col_double()))

# write_csv(groupid_24noout, "data/cmf/groupid_24_noout.csv")

groupid_24 <- read_csv("data/cmf/groupid_24_noout.csv")
...
#I have not rerun this to get rid of the outlier
...
length(unique(unlist(all24_clustlike))) == sum(lengths(all24_clustlike))
#each of these gene lists is unique

assign_24 <- unique(unlist(all24_clustlike))
all_genes <- rownames(hc_cpm_mat_24)
...
#removed the outlier that we had - PFOA 24hr H3
...
#this one has the outlier PFOA_24_H3 removed
...
cmf_mmr_old <- readRDS("data/cmf/subset/cormotif_fit_matrix_mmrk30.RDS")
...
# Rerun MMR Cormotif without Outlier

```{r MMR cormotif rerun plain}
#I have all of the factors already made for this, so I am going to go ahead with running CMF
#I haven't changed this one to have no outlier
...
# saveRDS(cmf_mmr_rm, "data/cmf/cormotif_fitmatrix_mmr_nooutlier_k20.RDS")

cmf_mmr <- readRDS("data/cmf/cormotif_fitmatrix_mmr_nooutlier_k20.RDS")

#plot the motifs and save them
plotIC(cmf_mmr)
plotMotif(cmf_mmr)
...
# Cluster Likelihood (clustlike) assignments
...
```{r UPR cormotif analysis, message=TRUE, warning=FALSE}
#start by looking at the cluster likelihood for each of these motifs

# cmf_upr <- readRDS("data/cmf/subset/cormotif_fit_matrix_uprk30.RDS")

#initialize list for my genes for each cluster
upr_clustlike <- list(
  upr_c1 = character(0),
  upr_c2 = character(0),
  upr_c3 = character(0),
...
#copy the clustlike matrix
upr_prob_df <-as.data.frame(upr_clust_prob)

#keep track of genes already assigned
assigned_genes_upr <- character(0)

#motif 1
clust1_upr <- rownames(upr_prob_df)[
  upr_prob_df$V1 > 0.6 &
  upr_prob_df$V2 < 0.5 &
  upr_prob_df$V3 < 0.5 &
  upr_prob_df$V4 < 0.5
]
clust1_upr <- setdiff(clust1_upr, assigned_genes_upr)
upr_clustlike$upr_c1 <- clust1_upr
assigned_genes_upr <- c(assigned_genes_upr, clust1_upr)

# motif 2
clust2_upr <- rownames(upr_prob_df)[
  upr_prob_df$V1 < 0.5 &
  upr_prob_df$V2 > 0.01 &
  upr_prob_df$V3 < 0.5 &
  upr_prob_df$V4 < 0.5
]
clust2_upr <- setdiff(clust2_upr, assigned_genes_upr)
upr_clustlike$upr_c2 <- clust2_upr
assigned_genes_upr <- c(assigned_genes_upr, clust2_upr)

# motif 3
clust3_upr <- rownames(upr_prob_df)[
  upr_prob_df$V1 < 0.5 &
  upr_prob_df$V2 < 0.5 &
  upr_prob_df$V3 > 0.01 &
  upr_prob_df$V4 < 0.5
]
clust3_upr <- setdiff(clust3_upr, assigned_genes_upr)
upr_clustlike$upr_c3 <- clust3_upr
assigned_genes_upr <- c(assigned_genes_upr, clust3_upr)

# motif 4
clust4_upr <- rownames(upr_prob_df)[
  upr_prob_df$V1 < 0.5 &
  upr_prob_df$V2 < 0.5 &
  upr_prob_df$V3 < 0.5 &
  upr_prob_df$V4 > 0.3
]
clust4_upr <- setdiff(clust4_upr, assigned_genes_upr)
upr_clustlike$upr_c4 <- clust4_upr
assign_upr <- c(assigned_genes_upr, clust4_upr)
length(assign_upr)
unassign_upr <- setdiff(all_genes, assign_upr)
length(unassign_upr)
#156 genes unassigned
...
#initialize list for DDR motifs
ddr_clustlike <- list(
  ddr_c1 = character(0),
  ddr_c2 = character(0),
  ddr_c3 = character(0),
...
#copy clustlike matrix to a dataframe
ddr_prob_df <- as.data.frame(ddr_clust_prob)

#keep track of genes already assigned
assigned_genes_ddr <- character(0)

#assign genes to motifs/clusters
#motif 1
clust1_ddr <- rownames(ddr_prob_df)[
  ddr_prob_df$V1 > 0.6 &
  ddr_prob_df$V2 < 0.5 &
  ddr_prob_df$V3 < 0.5 &
  ddr_prob_df$V4 < 0.5
]
clust1_ddr <- setdiff(clust1_ddr, assigned_genes_ddr)
ddr_clustlike$ddr_c1 <- clust1_ddr
assigned_genes_ddr <- c(assigned_genes_ddr, clust1_ddr)

#motif 2
clust2_ddr <- rownames(ddr_prob_df)[
  ddr_prob_df$V1 < 0.5 &
  ddr_prob_df$V2 > 0.05 &
  ddr_prob_df$V3 < 0.5 &
  ddr_prob_df$V4 < 0.5
]
clust2_ddr <- setdiff(clust2_ddr, assigned_genes_ddr)
ddr_clustlike$ddr_c2 <- clust2_ddr
assigned_genes_ddr <- c(assigned_genes_ddr, clust2_ddr)

#motif 3
clust3_ddr <- rownames(ddr_prob_df)[
  ddr_prob_df$V1 < 0.5 &
  ddr_prob_df$V2 < 0.5 &
  ddr_prob_df$V3 > 0.05 &
  ddr_prob_df$V4 < 0.5
]
clust3_ddr <- setdiff(clust3_ddr, assigned_genes_ddr)
ddr_clustlike$ddr_c3 <- clust3_ddr
assigned_genes_ddr <- c(assigned_genes_ddr, clust3_ddr)

#motif 4
clust4_ddr <- rownames(ddr_prob_df)[
  ddr_prob_df$V1 < 0.5 &
  ddr_prob_df$V2 < 0.5 &
  ddr_prob_df$V3 < 0.5 &
  ddr_prob_df$V4 > 0.05
]
clust4_ddr <- setdiff(clust4_ddr, assigned_genes_ddr)
ddr_clustlike$ddr_c4 <- clust4_ddr
assign_ddr <- c(assigned_genes_ddr, clust4_ddr)
unassign_ddr <- setdiff(all_genes, assign_ddr)
length(unassign_ddr)

#check totals
length(unique(unlist(ddr_clustlike)))        #number of genes assigned to motifs
length(all_genes) - length(unique(unlist(ddr_clustlike)))  #nunassigned genes
...
#initialize list for IMR motifs
imr_clustlike <- list(
  imr_c1 = character(0),
  imr_c2 = character(0)
)

#copy clustlike probabilities to dataframe
imr_prob_df <- as.data.frame(cmf_imr$bestmotif$clustlike)
rownames(imr_prob_df) <- rownames(expr_imr)

#track assigned genes for mutual exclusivity
assigned_genes_imr <- character(0)

#motif 1
clust1_imr <- rownames(imr_prob_df)[
  imr_prob_df$V1 > 0.8 & 
  imr_prob_df$V2 < 0.5
]
clust1_imr <- setdiff(clust1_imr, assigned_genes_imr)
imr_clustlike$imr_c1 <- clust1_imr
assigned_genes_imr <- c(assigned_genes_imr, clust1_imr)

#motif 2
clust2_imr <- rownames(imr_prob_df)[
  imr_prob_df$V1 < 0.5 & 
  imr_prob_df$V2 > 0.5
]
clust2_imr <- setdiff(clust2_imr, assigned_genes_imr)
imr_clustlike$imr_c2 <- clust2_imr
assign_imr <- c(assigned_genes_imr, clust2_imr)
unassign_imr <- setdiff(all_genes, assign_imr)
length(unassign_imr)

#check totals
length(unique(unlist(imr_clustlike)))      #number of assigned genes
length(all_genes) - length(unique(unlist(imr_clustlike)))  #unassigned genes
...
#initialize list for MMR motif
mmr_clustlike <- list(
  mmr_c1 = character(0)
)

#copy clustlike probabilities to dataframe
mmr_prob_df <- as.data.frame(cmf_mmr$bestmotif$clustlike)
rownames(mmr_prob_df) <- rownames(expr_mmr)

#track assigned genes (mutual exclusivity)
assigned_genes_mmr <- character(0)

#motif 1
clust1_mmr <- rownames(mmr_prob_df)[
  mmr_prob_df$V1 > 0.6
]
clust1_mmr <- setdiff(clust1_mmr, assigned_genes_mmr)
mmr_clustlike$mmr_c1 <- clust1_mmr
assign_mmr <- c(assigned_genes_mmr, clust1_mmr)
unassign_mmr <- setdiff(all_genes, assign_mmr)
length(unassign_mmr)

#check totals
length(unique(unlist(mmr_clustlike)))      #should equal number of assigned genes
length(all_genes) - length(unique(unlist(mmr_clustlike)))  #unassigned genes
```


# Use posterior probability to assign motif genes

## 24hr All Samples Subset
...
prob_filt_24_1 <- rownames(gene_probs24[(gene_probs24[,1] < 0.5
                                         & gene_probs24[,2] < 0.5 
                                         & gene_probs24[,3] < 0.5
                                         & gene_probs24[,4] < 0.5
                                         & gene_probs24[,5] < 0.5
                                         ...
                                         & gene_probs24[,14] < 0.5
                                         & gene_probs24[,15] < 0.5
                                         & gene_probs24[,16] < 0.5),]) #all white

length(prob_filt_24_1)
#7,094 genes

#intersect the clustlike and p.post together
fin_24_1 <- intersect(clust1_24, prob_filt_24_1)
cat("Number of genes passing both filters:", length(fin_24_1), "\n")

#6,818 genes pass both filters here

prob_filt_24_2 <- rownames(gene_probs24[(gene_probs24[,1] < 0.5
                                         & gene_probs24[,2] < 0.5 
                                         & gene_probs24[,3] < 0.5
                                         & gene_probs24[,4] < 0.5
                                         & gene_probs24[,5] < 0.5
                                         ...
                                         & gene_probs24[,15] > 0.1
                                         & gene_probs24[,16] > 0.5),])
length(prob_filt_24_2)
#1048 genes

#intersect the clustlike and p.post together
fin_24_2 <- intersect(clust2_24, prob_filt_24_2)
cat("Number of genes passing both filters:", length(fin_24_2), "\n")

prob_filt_24_3 <- rownames(gene_probs24[(gene_probs24[,1] > 0.5
                                         & gene_probs24[,2] > 0.5 
                                         & gene_probs24[,3] > 0.5
                                         & gene_probs24[,4] > 0.5
                                         & gene_probs24[,5] > 0.2
                                         ...
                                         & gene_probs24[,15] > 0.2
                                         & gene_probs24[,16] < 0.5),])
length(prob_filt_24_3)
#381 genes

#intersect the clustlike and p.post together
fin_24_3 <- intersect(clust3_24, prob_filt_24_3)
cat("Number of genes passing both filters:", length(fin_24_3), "\n")

prob_filt_24_4 <- rownames(gene_probs24[(gene_probs24[,1] < 0.5
                                         & gene_probs24[,2] < 0.5 
                                         & gene_probs24[,3] < 0.5
                                         & gene_probs24[,4] < 0.5
                                         & gene_probs24[,5] > 0.3
                                         ...
                                         & gene_probs24[,15] > 0.1
                                         & gene_probs24[,16] > 0.1),])
length(prob_filt_24_4)
#14 genes

#intersect the clustlike and p.post together
fin_24_4 <- intersect(clust4_24, prob_filt_24_4)
cat("Number of genes passing both filters:", length(fin_24_4), "\n")


prob_filt_24_5 <- rownames(gene_probs24[(gene_probs24[,1] > 0.3
                                         & gene_probs24[,2] > 0.5 
                                         & gene_probs24[,3] > 0.1
                                         & gene_probs24[,4] > 0.3
                                         & gene_probs24[,5] > 0.3
                                         ...
                                         & gene_probs24[,15] > 0.5
                                         & gene_probs24[,16] > 0.2),])
length(prob_filt_24_5)
#3 genes

#intersect the clustlike and p.post together
fin_24_5 <- intersect(clust5_24, prob_filt_24_5)
cat("Number of genes passing both filters:", length(fin_24_5), "\n")


prob_filt_24_6 <- rownames(gene_probs24[(gene_probs24[,1] > 0.5
                                         & gene_probs24[,2] > 0.5 
                                         & gene_probs24[,3] > 0.5
                                         & gene_probs24[,4] > 0.5
                                         & gene_probs24[,5] > 0.2
                                         ...
                                         & gene_probs24[,15] > 0.2
                                         & gene_probs24[,16] < 0.5),])
length(prob_filt_24_6)
#200 genes

#intersect the clustlike and p.post together
fin_24_6 <- intersect(clust6_24, prob_filt_24_6)
cat("Number of genes passing both filters:", length(fin_24_6), "\n")
#196 genes pass both

#now make a list of these gene sets for later analysis
all24_clustpost <- list(
  all24_c1 = fin_24_1,
  all24_c2 = fin_24_2,
  all24_c3 = fin_24_3,
  all24_c4 = fin_24_4,
...
length(all24_clustpost) == length(unique(all24_clustpost))
#each of these gene lists is unique

assign_24_cp <- unique(unlist(all24_clustpost))
length(assign_24_cp)
#8,431 genes assigned
all_genes <- rownames(hc_cpm_matrix_24)
unassign_24_cp <- setdiff(all_genes, assign_24_cp)
length(unassign_24_cp)
#5,308 genes unassigned
...
```{r p.post and clustlike together upr, warning=FALSE, message=FALSE}
...
length(upr_clustpost) == length(unique(upr_clustpost))
#each of these gene lists is unique

assign_upr_cp <- unique(unlist(upr_clustpost))
length(assign_upr_cp)
#13,569 genes assigned
all_genes <- rownames(hc_cpm_matrix_24)
unassign_upr_cp <- setdiff(all_genes, assign_upr_cp)
length(unassign_upr_cp)
#170 genes unassigned
...
#now make a list of these gene sets for later analysis
upr_clust_ppost_all <- list(
  clust1_pp_upr_all = fin_upr_1_all,
  clust2_pp_upr_all = fin_upr_2_all,
  clust3_pp_upr_all = fin_upr_3_all,
  clust4_pp_upr_all = fin_upr_4_all
)
...
length(ddr_clustpost) == length(unique(ddr_clustpost))
#each of these gene lists is unique

assign_ddr_cp <- unique(unlist(ddr_clustpost))
length(assign_ddr_cp)
#13,191 genes assigned
all_genes <- rownames(hc_cpm_matrix_24)
unassign_ddr_cp <- setdiff(all_genes, assign_ddr_cp)
length(unassign_ddr_cp)
#548 genes unassigned
...
prob_filt_imr_1 <- rownames(imr_gene_prob[(imr_gene_prob[,1] < 0.5
                                           & imr_gene_prob[,2] < 0.5
                                           & imr_gene_prob[,3] < 0.5
                                           & imr_gene_prob[,4] < 0.5),]) #all white boxes
length(prob_filt_imr_1)
#13,655 genes

#intersect the clustlike and p.post together
fin_imr_1 <- intersect(clust1_imr, prob_filt_imr_1)
cat("Number of genes passing both filters:", length(fin_imr_1), "\n")
#13,603 genes passing both


prob_filt_imr_2 <- rownames(imr_gene_prob[(imr_gene_prob[,1] > 0.3 #lighter grey box
                                           & imr_gene_prob[,2] > 0.5
                                           & imr_gene_prob[,3] > 0.5
                                           & imr_gene_prob[,4] > 0.5),])
length(prob_filt_imr_2)
#73 genes

#intersect the clustlike and p.post together
fin_imr_2 <- intersect(clust2_imr, prob_filt_imr_2)
cat("Number of genes passing both filters:", length(fin_imr_2), "\n")
#73 genes passing both
...
length(imr_clustpost) == length(unique(imr_clustpost))
#each of these gene lists is unique

assign_imr_cp <- unique(unlist(imr_clustpost))
length(assign_imr_cp)
#13,676 genes assigned
all_genes <- rownames(hc_cpm_matrix_24)
unassign_imr_cp <- setdiff(all_genes, assign_imr_cp)
length(unassign_imr_cp)
#63 genes unassigned
...
prob_filt_mmr_1 <- rownames(mmr_gene_prob[(mmr_gene_prob[,1] > 0.5
                                           & mmr_gene_prob[,2] > 0.2
                                           & mmr_gene_prob[,3] > 0.5
                                           & mmr_gene_prob[,4] < 0.5),]) 
#the p.post for this set is suuuper low so nothing will pop up for this, it's basically non-response
length(prob_filt_mmr_1)
#0 genes

#intersect the clustlike and p.post together
fin_mmr_1 <- intersect(clust1_mmr, prob_filt_mmr_1)
cat("Number of genes passing both filters:", length(fin_mmr_1), "\n")
#0 genes passing both

mmr_clustpost <- list(
  mmr_c1 = fin_mmr_1
)

assign_mmr_cp <- unique(unlist(mmr_clustpost))
length(assign_mmr_cp)
#0 genes assigned
all_genes <- rownames(hc_cpm_matrix_24)
unassign_mmr_cp <- setdiff(all_genes, assign_mmr_cp)
length(unassign_mmr_cp)
#13,739 genes unassigned

```



# Barplots and Pie Charts Subset Motifs Clustlike vs Clustlike + p.post

```{r generate stacked barplots and pie charts for each subset, message=FALSE, warning=FALSE}

#now that I've assigned genes to each motif for both clustlike alone and for clustlike and p.post togehter, let's take a look at their distribution

#make a loop for a stacked barplot that goes through each of my gene lists to plot them

subset_names <- c("upr", "upr_cp", "ddr", "ddr_cp", "imr", "imr_cp", "mmr", "mmr_cp")

subset_registry <- list(
  upr = list(
    assign = upr_clustlike,
    unassign = unassign_upr
  ),
  upr_cp = list(
    assign = upr_clustpost,
    unassign = unassign_upr_cp
  ),
  ddr = list(
    assign = ddr_clustlike,
    unassign = unassign_ddr
  ),
  ddr_cp = list(
    assign = ddr_clustpost,
    unassign = unassign_ddr_cp
  ),
  imr = list(
    assign = imr_clustlike,
    unassign = unassign_imr
  ),
  imr_cp = list(
    assign = imr_clustpost,
    unassign = unassign_imr_cp
  ),
  mmr = list(
    assign = mmr_clustlike,
    unassign = unassign_mmr
  ),
  mmr_cp = list(
    assign = mmr_clustpost,
    unassign = unassign_mmr_cp
  )
...
for (subset in names(subset_registry)) {
  ...
  res <- cmf_plots_funct(
    assign = subset_registry[[subset]]$assign,
...
# Venn Diagrams of Motif Clusters

```{r motif venn diagrams, message=FALSE, warning=FALSE}
#I want to make venn diagrams between each of my motif sets to ensure that there is no overlap between them

#define subset_clustlike
subset_clustlike <- list(
  upr = upr_clustlike,
  upr_cp = upr_clustpost,
  ddr = ddr_clustlike,
  ddr_cp = ddr_clustpost,
  imr = imr_clustlike,
  imr_cp = imr_clustpost,
  mmr = mmr_clustlike
)
...
for (subset_name in names(subset_clustlike)) {
  ...
  # drop empty motifs
  motif_list <- motif_list[sapply(motif_list, length) > 0]
  
  # need at least 2 motifs for a Venn
  if (length(motif_list) < 2) {
    message("  Skipping ", subset_name, ": <2 motifs")
    next
  }
  ...
  p <- ggVennDiagram(
    motif_list,
    category.names = names(motif_list)
  ) +
    ggtitle(paste0("Cormotif motifs: ", toupper(subset_name))) +
    theme(
  ...
  save_plot(
    plot     = p,
    filename = paste0("Venn_", subset_name),
    folder   = output_folder,
    width    = 8,
    height   = 6
  )
...
#convert hc_cpm_mat into a df for this
hc_cpm_mat <- hc_cpm_mat %>% as.data.frame()
...
# Response Genes Compared Clustlike vs Clust+p.Post

```{r plot response genes clust vs clustppost, message=FALSE, warning=FALSE}
#ensure expression matrix is a data.frame
hc_cpm_mat <- hc_cpm_mat %>% as.data.frame()
...
#pull expression for a FIXED gene set
process_motif_genes <- function(example_gene_ids, expr_mat, meta_df, cluster_label) {
  
  # Keep only genes present in the expression matrix
  example_gene_ids <- example_gene_ids[example_gene_ids %in% expr_mat$Entrez_ID]
  if(length(example_gene_ids) == 0) return(NULL)
  
  gene_mat <- expr_mat[expr_mat$Entrez_ID %in% example_gene_ids, , drop = FALSE]
  ...
  long_data <- gene_mat %>%
    ...
    left_join(meta_df, by = c("Sample" = "Final_sample_name")) %>%
    mutate(ClusterType = cluster_label)
...
plot_example_gene <- function(gene_data, gene_name, motif_name) {
  ...
  ggplot(gene_data, aes(x = Cond, y = log2cpm, fill = Stimulus)) +
    geom_boxplot(outlier.shape = NA) +
    ...
    facet_wrap(~ClusterType, nrow = 1) +
    ggtitle(paste("log2cpm expression", gene_name, motif_name)) +
    labs(x = "Condition", y = "log2cpm") +
    theme_custom()
}
...
#find genes from each motif to plot for both sets
example_gene_lookup <- list()

for(subset_name in names(subset_clustpost)) {
  
  motif_list <- subset_clustpost[[subset_name]]
  
  for(motif_name in names(motif_list)) {
    
    genes <- motif_list[[motif_name]]
    genes <- intersect(genes, hc_cpm_mat$Entrez_ID)
    
    if(length(genes) == 0) next
    
    example_gene_lookup[[paste0(subset_name, "_", motif_name)]] <-
      sample(genes, size = min(5, length(genes)))
  }
}

#sample example genes ONCE per motif
ex_gene_plots <- list()
...
for(subset_name in names(subset_clustlike)) {
  ...
  motif_list_clustlike <- subset_clustlike[[subset_name]]
  motif_list_clustpost  <- subset_clustpost[[subset_name]]
  subset_rules     <- subset_stimulus_map[[subset_name]]
  
  for(motif_name in names(motif_list_clustlike)) {
    
    #skip if no example genes
    key <- paste0(subset_name, "_", motif_name)
    if(!key %in% names(example_gene_lookup)) next
    example_gene_ids <- example_gene_lookup[[key]]
    
    #pull clustlike genes
    long_clustlike <- process_motif_genes(
      example_gene_ids,
      hc_cpm_mat,
      meta_cmf,
      cluster_label = "clustlike"
    )
    
    # Pull clustpost genes (only if motif exists in clustpost)
    long_clustpost <- NULL
    if(!is.null(motif_list_clustpost)) {
      genes_clustpost <- motif_list_clustpost[[motif_name]]
      genes_clustpost <- intersect(genes_clustpost, example_gene_ids)
      if(length(genes_clustpost) > 0) {
        long_clustpost <- process_motif_genes(
          genes_clustpost,
          hc_cpm_mat,
          meta_cmf,
          cluster_label = "clustlike + p.post"
        )
      }
    }
    
    # Combine for faceting
    long_gene_data <- bind_rows(long_clustlike, long_clustpost)
    if(is.null(long_gene_data) || nrow(long_gene_data) == 0) next
    
    # Subset for stimulus and time
    long_gene_data <- long_gene_data[
      long_gene_data$Stimulus %in% subset_rules$stimuli &
      long_gene_data$Time     %in% subset_rules$times,
    ]
    ...
    # Factor ordering
    long_gene_data$Stimulus <- factor(long_gene_data$Stimulus, 
                                      levels = stimulus_order)
    long_gene_data$Species  <- factor(long_gene_data$Species, 
                                      levels = species_order)
    long_gene_data$Time     <- factor(long_gene_data$Time,
                                      levels = time_order)
    long_gene_data$Cond     <- factor(long_gene_data$Cond, 
                                      levels = cond_order)
    long_gene_data$Ind      <- factor(long_gene_data$Ind,
                                      levels = ind_order)
...
ttbl_list_24 <- readRDS("data/de/toptable_list_24hr_updated_symbolens.RDS")
...
motif_stimuli <- list(
  all = c("TUN", "THA", "DOX", "NUTL", "LPS", "TNFa","BPA", "PFOA"),
  upr = c("TUN", "THA"),
  ddr = c("DOX", "NUTL"),
  imr = c("LPS", "TNFa")
)
...
plot_logFC_distribution <- function(df, y_var = c("logFC", "absFC"), title, stim_col, resp_group) {
  ...
  cond_levels <- list(
  all24 = c("TUN_24_H", "THA_24_H", "TUN_24_C", "THA_24_C", "DOX_24_H", "NUTL_24_H", "DOX_24_C", "NUTL_24_C", "LPS_24_H", "TNFa_24_H", "LPS_24_C", "TNFa_24_C", "BPA_24_H", "PFOA_24_H", "BPA_24_C", "PFOA_24_C")
...
#logFC distributions clustlike vs clustlike + p.post sets

```{r logFC distribtions clustlike vs clustpost, warning=FALSE, message=FALSE}
#I want to make a function to loop through my different subsets to get an idea of their logFC distribution
#this will be using my toptables that I made with DE analysis previously
...
#define your motif gene sets for each response group
motif_gene_sets <- list(
  upr    = upr_clustlike,
  upr_cp = upr_clustpost,
  ddr    = ddr_clustlike,
  ...
  imr_cp = imr_clustpost,
  mmr    = mmr_clustlike,
  mmr_cp = mmr_clustpost
)

#also define which stimuli correspond to each response group
motif_stimuli <- list(
  upr    = c("TUN", "THA"),
  upr_cp = c("TUN", "THA"),
  ddr    = c("DOX", "NUTL"),
...
#define the fixed condition order for each response group
cond_levels <- list(
  upr    = c("TUN_24_H", "TUN_24_C", "THA_24_H", "THA_24_C"),
  upr_cp = c("TUN_24_H", "TUN_24_C", "THA_24_H", "THA_24_C"),
  ddr    = c("DOX_24_H", "DOX_24_C", "NUTL_24_H", "NUTL_24_C"),
  ddr_cp = c("DOX_24_H", "DOX_24_C", "NUTL_24_H", "NUTL_24_C"),
  imr    = c("LPS_24_H", "LPS_24_C",  "TNFa_24_H","TNFa_24_C"),
  imr_cp = c("LPS_24_H", "LPS_24_C", "TNFa_24_H","TNFa_24_C"),
  mmr    = c("BPA_24_H", "BPA_24_C", "PFOA_24_H", "PFOA_24_C"),
  mmr_cp = c("BPA_24_H", "BPA_24_C", "PFOA_24_H", "PFOA_24_C")
)


#function to calculate medians per Condition + ClusterType
compute_medians <- function(df, y_var) {
  df %>%
    group_by(Condition, ClusterType) %>%
    summarise(Median = median(.data[[y_var]], na.rm = TRUE), .groups = "drop")
}

#faceted plotting function
plot_logFC_distribution_facet <- function(df, y_var = c("logFC", "absFC"), title, stim_col) {
  y_var <- match.arg(y_var)
  y_lab <- ifelse(y_var == "logFC", "logFC", "|logFC|")
  
  medians <- compute_medians(df, y_var)
  
  ggplot(df, aes(x = Condition, y = .data[[y_var]], fill = Stimulus)) +
    geom_boxplot(outlier.shape = NA) +
    scale_fill_manual(values = stim_col) +
    facet_wrap(~ClusterType, nrow = 1, scales = "free_x") +
    geom_text(data = medians, aes(x = Condition, y = Median, label = round(Median, 2)),
              vjust = -0.5, inherit.aes = FALSE, size = 3) +
    labs(x = "Condition", y = y_lab, title = title) +
    theme_custom()
}

#loop through response groups
for (resp in names(motif_gene_sets)) {
  ...
  ttbl_resp <- combined_ttbl_24 %>%
    filter(Stimulus %in% stim_prefix) %>%
    mutate(
      Stimulus  = factor(Stimulus, levels = stim_prefix),
      Condition = paste0(Stimulus, "_24_", Species)
    )
  ...
  for (motif_name in names(motif_list)) {
    ...
    #pull genes from clustlike and clustpost
    genes_like <- if(!is.null(subset_clustlike[[resp]])) subset_clustlike[[resp]][[motif_name]] else NULL
    genes_post <- if(!is.null(subset_clustpost[[resp]])) subset_clustpost[[resp]][[motif_name]] else NULL
    
    #keep only genes present in DE table
    genes_like <- intersect(genes_like, ttbl_resp$Entrez_ID)
    genes_post <- intersect(genes_post, ttbl_resp$Entrez_ID)
    
    if (length(genes_like) == 0 & length(genes_post) == 0) next
    
    #combine into a single data frame with ClusterType label
    df_like <- ttbl_resp %>% filter(Entrez_ID %in% genes_like) %>% mutate(ClusterType = "clustlike", absFC = abs(logFC))
    df_post <- ttbl_resp %>% filter(Entrez_ID %in% genes_post) %>% mutate(ClusterType = "clustlike + p.post", absFC = abs(logFC))
    
    df_motif <- bind_rows(df_like, df_post)
    n_genes <- n_distinct(df_motif$Entrez_ID)
    
    #plot logFC
    p_logFC <- plot_logFC_distribution_facet(df_motif, y_var = "logFC",
                                             title = paste0(motif_name, " logFC (n = ", n_genes, ")"),
                                             stim_col = stim_col[stim_prefix])
    ...
    save_plot(
      filename = paste0("Motif_", motif_name, "_logFC_facet"),
      plot     = p_logFC,
      folder   = output_folder,
      width    = 12,
      height   = 8
    ...
    #plot abs(logFC)
    p_absFC <- plot_logFC_distribution_facet(df_motif, y_var = "absFC",
                                             title = paste0(motif_name, " |logFC| (n = ", n_genes, ")"),
                                             stim_col = stim_col[stim_prefix])
    ...
    save_plot(
      filename = paste0("Motif_", motif_name, "_abslogFC_facet"),
      plot     = p_absFC,
      folder   = output_folder,
      width    = 12,
      height   = 8
...
# GO KEGG Pathway Analysis All Motifs


```{r GO KEGG all motifs, message=FALSE, warning=FALSE}
#do pathway analysis for both of my motif groups to visualize the biological pathways associated with each set 
motif_sets_clustlike <- list(
  upr = upr_clustlike,
  ddr = ddr_clustlike,
  imr = imr_clustlike,
  mmr = mmr_clustlike
...
#make list containers for the plots
gost_results_clustlike <- list()
gost_results_clustpost <- list()

gost_plots_clustlike   <- list()
gost_plots_clustpost   <- list()

#make a gost plot function for gobp and kegg
...
#run one motif set
run_gost_for_set <- function(motif_sets, set_label) {

  gost_results <- list()
  gost_plots   <- list()

  for (resp in names(motif_sets)) {

    message("Processing ", set_label, " response group: ", resp)

    for (motif_name in names(motif_sets[[resp]])) {

      genes <- motif_sets[[resp]][[motif_name]]
      message("  Motif ", motif_name, " (n = ", length(genes), ")")

      gost_res <- run_gost_safe(genes, all_genes)
      ...
      gost_results[[resp]][[motif_name]] <- res_tbl

      ## ---- GO:BP ----
      go_tbl <- res_tbl %>% filter(source == "GO:BP")
      if (nrow(go_tbl) > 0) {
        p_go <- plot_enrichment_dotplot(
          go_tbl,
          title = paste0(
            toupper(resp), " ", motif_name,
            " (", set_label, ") GO:BP"
          ),
          ylab_txt = "GO:BP term"
        )

        plot_name <- paste0(resp, "_", motif_name, "_", set_label, "_GO_BP")

        gost_plots[[plot_name]] <- p_go
        print(p_go)
      ...
      ## ---- KEGG ----
      kegg_tbl <- res_tbl %>% filter(source == "KEGG")
      if (nrow(kegg_tbl) > 0) {
        p_kegg <- plot_enrichment_dotplot(
          kegg_tbl,
          title = paste0(
            toupper(resp), " ", motif_name,
            " (", set_label, ") KEGG"
        ...
        plot_name <- paste0(resp, "_", motif_name, "_", set_label, "_KEGG")

        gost_plots[[plot_name]] <- p_kegg
        print(p_kegg)
...
#run clustlike
clustlike_out <- run_gost_for_set(motif_sets_clustlike, "clustlike")
# gost_results_clustlike <- clustlike_out$results
# gost_plots_clustlike   <- clustlike_out$plots

#run clustpost
clustpost_out <- run_gost_for_set(motif_sets_clustpost, "clustpost")
# gost_results_clustpost <- clustpost_out$results
# gost_plots_clustpost   <- clustpost_out$plots

#print the plots so I can visualize them
for (grp in names(gost_plots_clustlike)) {
  for (nm in names(gost_plots_clustlike[[grp]])) {
    print(gost_plots_clustlike[[grp]][[nm]])
  }
}

#print the clustpost plots as well
for (grp in names(gost_plots_clustpost)) {
  for (nm in names(gost_plots_clustpost[[grp]])) {
    print(gost_plots_clustpost[[grp]][[nm]])
  }
}

#save plots for clustlike and clustpost
for (grp in names(gost_plots_clustlike)) {
  for (nm in names(gost_plots_clustlike[[grp]])) {
    save_plot(
      filename = paste0("clustlike_", grp, "_", nm),
      plot     = gost_plots_clustlike[[grp]][[nm]],
      folder   = output_folder,
      width    = 6,
      height   = 8
    )
...
for (grp in names(gost_plots_clustpost)) {
  for (nm in names(gost_plots_clustpost[[grp]])) {
    save_plot(
      filename = paste0("clustpost_", grp, "_", nm),
      plot     = gost_plots_clustpost[[grp]][[nm]],
      folder   = output_folder,
      width    = 6,
      height   = 8
    )
...
```{r overlap GWAS with cluster gene lists, warning=FALSE, message=FALSE, include=FALSE}

## Overlap GWAS genes with Clusters


# I want to take my HF and CVD GWAS associations obtained above and look at the proportion of these genes which overlap with my cluster gene lists across subsets.
...
meta_cmf <- readRDS("data/cmf/metadata_cormotif_reordered.RDS") %>% as.data.frame()
rownames(meta_cmf) <- colnames(expr)
# save(meta_cmf, file = "meta_cmf_reordered_all.RData")
#now that the metadata dataframe is reordered, use it to reorder my expr matrix
...
expr <- as.matrix(expr)
# --- Everything from here to Run Model is setting up my groupid and compid ---
# --- Align metadata and log2cpm
metadata <- metadata[match(colnames(log2cpm), metadata$Final_sample_name), ]

# --- Build group and comp IDs from metadata ---
metadata$group <- paste(metadata$Condition, metadata$Species, sep = "_")

condition_order <- c("TUN_2",
                "THA_2",
                "DOX_2",
                "NUTL_2",
                "DMSO_2",
...
group_levels <- c(paste(condition_order, "H", sep = "_"), paste(condition_order, "C", sep = "_"))

metadata$group <- factor(metadata$group, levels = group_levels, ordered = TRUE)

groupid <- as.numeric(metadata$group)

metadata$groupid <- as.numeric(metadata$group)

# 1. Define your fixed stimulus order and vehicle mapping
stim_order <- c("TUN", "THA", "DOX", "NUTL", "LPS", "TNFa", "BPA", "PFOA")
vehicle_map <- c(TUN="DMSO", THA="DMSO", DOX="DMSO", NUTL="DMSO",
                 LPS="H2O", TNFa="H2O",
                 BPA="EtOH", PFOA="EtOH")

# 2. Define the four global blocks you requested
# Block 1: 2hr Human
# Block 2: 2hr Chimpanzee
# Block 3: 24hr Human
# Block 4: 24hr Chimpanzee
blocks <- list(
  list(time="2",  spec="H"),
  list(time="2",  spec="C"),
  list(time="24", spec="H"),
...
for(b in blocks) {
  for(stim in stim_order) {
    veh <- vehicle_map[stim]
    
    # Identify the factor level indices for Stimulus and its specific Vehicle
    stim_lvl <- paste(stim, b$time, b$spec, sep = "_")
    veh_lvl  <- paste(veh, b$time, b$spec, sep = "_")
    
    # Extract the numeric position from your metadata levels
    idx_stim <- which(levels(metadata$group) == stim_lvl)
    idx_veh  <- which(levels(metadata$group) == veh_lvl)
    
    # Store as a 2-row vector (Stimulus vs Vehicle)
    col_name <- paste(stim, b$spec, b$time, sep = "_")
    comp_list[[col_name]] <- c(idx_stim, idx_veh)
  }
}

# 4. Finalize the compid matrix
# Each column is a comparison, each row is the group index
final_compid <- do.call(cbind, comp_list)
