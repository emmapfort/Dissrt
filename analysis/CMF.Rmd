...
# Create gene dataframes FIX THIS SOON
...
#   mutate(Stimulus = factor(Stimulus, levels = c("24T", "24R", "144R"), labels = c("24hr", "24hr Recovery", "144hr Recovery"))) %>% 
...
subset_names <- c("all_24", "upr_all", "upr", "ddr", "imr", "mcr")
...
subset_registry <- list(
  ...
  mcr = list(
    clustlike = mcr_clustlike,
    unassign = unassign_mcr
  )
...
```{r Pull out example genes for each subset}
...
for(subset_name in names(subset_clustlike)) {
  ...
  for(motif_name in names(motif_list)) {
    ...
    #pull the genes
    long_gene_data <- process_motif_genes(motif_genes, hc_cpm_mat, meta_cmf, n_genes = 5)
    
    if(is.null(long_gene_data)) next
    
    #loop through each ex gene and plot
    for(gene in unique(long_gene_data$SYMBOL)) {
      gene_data <- long_gene_data %>% 
        filter(SYMBOL == gene)
      
      p <- plot_example_gene(gene_data, gene_name = gene, motif_name = motif_name)
      
      #save plot to list
      plot_name <- paste0("cmf_", subset_name, "_", motif_name, "_exgene_", gene)
      ex_gene_plots[[plot_name]] <- p
      
      #save plots using my custom function
      save_plot(
        plot = p,
        filename = plot_name,
        folder = output_folder,
