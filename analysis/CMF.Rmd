...
hc_cpm_matrix_24 <- readRDS("data/counts/")
...
gene_prob_all <- cmf_fit_24$bestmotif$p.post
rownames(gene_prob_all) <- rownames(expr_24)
gene_probs <- gene_prob_all

#assign each gene to a motif with max post prob
assigned_motifs <- apply(gene_probs, 1, which.max)
max_probs <- apply(gene_probs, 1, max)

#combine these into a dataframe - motif assigned genes (p.post)
motif_assignment_df <- gene_probs %>%
  as.data.frame() %>%
...
motif_prob <- cmf_fit_24$bestmotif$clustlike
rownames(motif_prob) <- rownames(gene_prob_all)
# write.csv(motif_prob,"data/cmf/cormotif_probability_genelist_all_RUV_log2cpm_NormCounts.csv")
...
length(unique(unlist(cmf_clustlike))) == sum(lengths(cmf_clustlike))
#each of these gene lists is unique

assigned_genes <- unique(unlist(cmf_clustlike))
all_genes <- rownames(hc_cpm_matrix_24)
clust_unassign <- setdiff(all_genes, assigned_genes)

#quick barplot and pie chart
clust_data <- data.frame(
  Category = c("1", "2", "3", "4", "5", "6", "unassign"), 
  Value = c(length(clust1_genes), length(clust2_genes), length(clust3_genes),
            length(clust4_genes), length(clust5_genes), length(clust6_genes),
            length(clust_unassign)
))
...
ggplot(clust_data, aes(x = "", y = Value, fill = Category)) +
...
clust_data %>% ggplot(aes(x = "", y = Value, fill = Category))+
...
filt_toptable_clust <- combined_24hr %>% 
  dplyr::filter(Entrez_ID  %in% clust1_genes) %>% 
  ...
  ylab("logFC")+
  theme_bw()+
  ggtitle("LogFC Motif 1 RUVs clustlike")+
...
clust1_mat <- as.matrix(clust1_genes)
...
#C2
clust2_mat <- as.matrix(clust2_genes)
...
#C3
clust3_mat <- as.matrix(clust3_genes)
...
#C2
clust4_mat <- as.matrix(clust4_genes)
...
#C5
clust5_mat <- as.matrix(clust5_genes)
...
#C6
clust6_mat <- as.matrix(clust6_genes)
...
```{r Subset my CMF results for each response category}
...
#mcr
expr_mcr <- expr[, grepl("^(BPA|PFOA|EtOH)_24_", colnames(expr))]
...
groupid_mcr    <- setNames(as.numeric(groupid_mcr),    names(groupid_mcr))
...
compid_mcr <- data.frame(Cond1 = c(1, 2, 3, 4),
                         Cond2 = c(5, 6, 5, 6))
...
```{r run cormotif fit for each subset}
...
cmf_mcr <- readRDS("data/cmf/subset/cormotif_fit_matrix_mcrk30.RDS")
...
clust1_upr <- 
  ...
  rownames
length(clust1_upr)
#11,172 genes w/ cutoff of 0.6 (too many at 0.5)
...
length(unique(unlist(upr_clustlike))) == sum(lengths(upr_clustlike))
#each of these gene lists is unique
...
length(unique(unlist(upr_clustlike_all))) == sum(lengths(upr_clustlike_all))
#each of these gene lists is unique
...
#13250 genes are assigned to a motif with these cutoffs presently
...
length(unique(unlist(ddr_clustlike))) == sum(lengths(ddr_clustlike))
#each of these gene lists is unique
...
clust1_imr <- 
  imr_clust_prob %>%
  as.data.frame() %>%
  filter(V1 > 0.6 & V2 < 0.5) %>% 
  rownames
length(clust1_imr)
#13643 genes w/ cutoff of 0.6 (too many at 0.5)
...
length(unique(unlist(imr_clustlike))) == sum(lengths(imr_clustlike))
#each of these gene lists is unique
...
#13727 genes are assigned to a motif with these cutoffs presently
#12 unassigned genes
```

#MCR Subset Motifs

```{r MCR cormotif analysis}

#extract the posterior probability that these genes belong to motifs
mcr_gene_prob <- cmf_mcr$bestmotif$p.post
rownames(mcr_gene_prob) <- rownames(expr_mcr)

#assign each gene to a motif with max post prob
mcr_assign_mtf <- apply(mcr_gene_prob, 1, which.max)
max_probs_mcr <- apply(mcr_gene_prob, 1, max)

#combine these into a dataframe - motif assigned genes (p.post)
mcr_clust_df <- mcr_gene_prob %>%
  as.data.frame() %>%
  rownames_to_column("Gene") %>%
  mutate(
    Assigned_Motif = mcr_assign_mtf[Gene],
    Max_Probability = max_probs_mcr[Gene]
  )
  
#extract the cluster likelihood - which DEGs are most likely to be in this cluster
mcr_clust_prob <- cmf_mcr$bestmotif$clustlike
rownames(mcr_clust_prob) <- rownames(mcr_gene_prob)

# write.csv(mcr_clust_prob,"data/cmf/subset/cormotif_mcr_clusterlikelihood.csv")
...
all_genes <- rownames(hc_cpm_matrix_24)
unassign_mcr <- setdiff(all_genes, clust1_mcr)
length(unassign_mcr)
