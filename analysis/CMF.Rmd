...
#13284 genes are assigned to a motif with these cutoffs presently
#455 unassigned genes
...
#there's nothing in this set, so we can disregard it for now.
...
subset_names <- c("upr", "upr_cp", "ddr", "ddr_cp", "imr", "imr_cp", "mmr")
...
```{r pull example genes from two sets, clustlike and clust with p.post filtering, warning=FALSE, message=FALSE}
...
#subset stimulus rules
subset_stimulus_map <- list(
  upr = list(stimuli = c("TUN", "THA", "DMSO"), times = c("24")),
  upr_cp = list(stimuli = c("TUN", "THA", "DMSO"), times = c("24")),
  ddr = list(stimuli = c("DOX", "NUTL", "DMSO"), times = c("24")),
  ddr_cp = list(stimuli = c("DOX", "NUTL", "DMSO"), times = c("24")),
  imr = list(stimuli = c("LPS", "TNFa", "H2O"), times = c("24")),
  imr_cp = list(stimuli = c("LPS", "TNFa", "H2O"), times = c("24")),
  mmr = list(stimuli = c("BPA", "PFOA", "EtOH"), times = c("24"))
)

#make a function to process through my gene lists and pick out 5 genes
process_motif_genes <- function(gene_vec, expr_mat, meta_df) {
...
plot_example_gene <- function(gene_data, gene_name, motif_name, label_suffix) {
  ...
  ggplot(gene_data, aes(x = Cond, y = log2cpm, fill = Stimulus)) +
    ...
    ggtitle(
      paste(
        "log2cpm expression", gene_name,
        "cluster", motif_name, label_suffix
      )
    ) +
    labs(x = "Condition", y = "log2cpm") +
    theme_custom()
...
#define cluster/motif gene lists
subset_clustlike <- list(
  upr = upr_clustlike,
  ddr = ddr_clustlike,
  imr = imr_clustlike,
...
for (subset_name in names(subset_clustlike)) {
  ...
  motif_list_clust <- subset_clustlike[[subset_name]]
  motif_list_cp    <- subset_clustpost[[subset_name]]
  subset_rules     <- subset_stimulus_map[[subset_name]]

  #skip if no clust+p.post version (mmr)
  if (is.null(motif_list_cp)) {
    cat("  Skipping clust+p.post for", subset_name, "\n")
    next
  }

  for (motif_name in names(motif_list_clust)) {

    genes_clust <- motif_list_clust[[motif_name]]
    genes_cp    <- motif_list_cp[[motif_name]]

    shared_genes <- intersect(genes_clust, genes_cp)
    shared_genes <- shared_genes[shared_genes %in% hc_cpm_mat$Entrez_ID]

    if (length(shared_genes) == 0) next

    n_pull <- min(5, length(shared_genes))
    example_genes <- sample(shared_genes, n_pull)

    for (set_type in c("clustlike", "clustpost")) {

      long_gene_data <- process_motif_genes(
        gene_vec = example_genes,
        expr_mat = hc_cpm_mat,
        meta_df  = meta_cmf
      )
      ...
      long_gene_data <- long_gene_data %>%
        filter(
          Stimulus %in% subset_rules$stimuli,
          Time %in% subset_rules$times,
          Entrez_ID %in% example_genes
        ) %>%
        mutate(
      ...
      for (gene in unique(long_gene_data$SYMBOL)) {
        ...
        p <- plot_example_gene(
          ...
          gene_name   = gene,
          motif_name  = motif_name,
          label_suffix = set_type
        )

        plot_name <- paste0(
          "cmf_", subset_name, "_", motif_name, "_",
          set_type, "_exgene_", gene
        )
...
```{r GO KEGG all motifs}
#now I want to assess the pathways associated with the genes in each of these gene lists per cluster

## ================================
## Pathway analysis for Cormotif motifs
## GO:BP + KEGG using gprofiler2
## ================================
...
## ----------------
## Helper: dotplot (MATCHES YOUR OLD THEME)
## ----------------
...
## ----------------
## Main loop: response group -> motif
## ----------------
gost_results <- list()
gost_plots   <- list()
...
for (resp in names(motif_gene_sets)) {
  ...
  gost_results[[resp]] <- list()
  gost_plots[[resp]]   <- list()
  
  for (motif_name in names(motif_gene_sets[[resp]])) {
    
    genes <- motif_gene_sets[[resp]][[motif_name]]
    ...
    if (nrow(go_tbl) > 0) {
      gost_plots[[resp]][[paste0(motif_name, "_GO_BP")]] <-
    ...
    if (nrow(kegg_tbl) > 0) {
      gost_plots[[resp]][[paste0(motif_name, "_KEGG")]] <-
        plot_enrichment_dotplot(
          kegg_tbl,
          title = paste0(
            toupper(resp), " ", motif_name,
...
## ----------------
## Example: save all plots
## ----------------
for (resp in names(gost_plots)) {
  for (nm in names(gost_plots[[resp]])) {
    ggsave(
      filename = paste0(resp, "_", nm, ".pdf"),
      plot     = gost_plots[[resp]][[nm]],
      width    = 7,
      height   = 5
    )
  }
