...
```{r Read in Metadata Sheet, message=FALSE, warning=FALSE}
...
```{r Join my Human and Chimp datasets together, message=FALSE, warning=FALSE}
...
```{r Remove extra samples, message=FALSE, warning=FALSE}
...
Perform PCA analysis on subset of data. At this stage, I removed an outlier sample PFOA 24hr H3 which was not clustering with the rest of these data.

```{r PCA Plots, warning=FALSE, message=FALSE, fig.width=8, fig.height=6}
...
hc_cpm_mat_24 <- readRDS("data/counts/hc_filtered_cpm_matrix_24hr_subset.RDS")
hc_cpm_mat_24 <- hc_cpm_mat_24 %>% 
  as.data.frame() %>% 
  dplyr::select(- PFOA_24_H3) 
