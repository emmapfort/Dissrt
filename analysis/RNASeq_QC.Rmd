...
theme_custom_small <- readRDS("data/plot_theme_custom.RDS")
...
#Datasets Creation
```{r Import Counts Tables, warning=FALSE, message=FALSE}
...
#Metadata
```{r Read in Metadata Sheet, warning=FALSE, message=FALSE}
...
#Combine my datasets
```{r Join my Human and Chimp datasets together, warning=FALSE, message=FALSE}
...
#QC Mapping/fC
```{r Mapping and Counts Stats, warning=FALSE, message=FALSE}
...
#Filter Dataframe
```{r Filter with rowMeans or log2cpm}
#first, convert to log2cpm

#ensure that EnsemblID are the rownames
...
#now filter by log2cpm cutoff of 1


#now try 1.5 log2cpm cutoff like Blake et al.
...
hist(tmm_cpm, main = "log2(CPM) values in unfiltered data (n = 307 samples)", breaks = 100, ylim = c(0, 50000), xlab = "log2(CPM) values")
...
hist(cpm_filtered, xlab = "Log2(CPM)", main = "Log2(CPM) values for genes meeting the filtering criteria", breaks = 100 )
...
# now move this above to where I was making cpm lists to have all three metrics
```
...
metadata_2$Ind <- factor(metadata_2$Ind, levels = unique(metadata_2$Ind))
metadata_2$Stimuli <- factor(metadata_2$Stimulus, levels = c("TUN", "THA", "DOX", "NUTL", "DMSO", "LPS", "TNFa", "H2O", "BPA", "PFOA", "EtOH"))
...
generate_volcano_plot <- function(toptable, title) {
  ...
  #check for entrezid 
  if(!"Ensembl_ID" %in% colnames(toptable)) stop("ENSEMBL col not present")
...
volcano_plots_sig <- lapply(names(ttbl_list), function(nm) {
  generate_volcano_plot(ttbl_list[[nm]], title = nm)
})
names(volcano_plots_sig) <- names(ttbl_list)
...
walk2(ttbl_list, names(ttbl_list), ~ {
...
for (st in stimuli) {
  ...
  deg_H <- ttbl_list[[nm_H]] %>%
    dplyr::filter(adj.P.Val < 0.05) %>%
    pull(Entrez_ID)
  deg_C <- ttbl_list[[nm_H]] %>%
    dplyr::filter(adj.P.Val < 0.05) %>%
    pull(Entrez_ID)
  ...
  png_filename <- file.path(output_folder,
                            paste0("PropVenn_", st, "_EMP_260516.png"))
...
```{r venn try two}
stimuli <- c(
             "TUN_24", "THA_24", "DOX_24", "NUTL_24",
             "LPS_24", "TNFa_24", "BPA_24", "PFOA_24")
...
for (st in stimuli) {
  ...
  # Pull Human DEGs and wrap in unique()
  h_genes <- if (nm_H %in% names(ttbl_list)) {
    df_h <- ttbl_list[[nm_H]]
    df_h$Ensembl_ID <- as.character(df_h$Ensembl_ID)
    unique(df_h$Ensembl_ID[!is.na(df_h$adj.P.Val) & df_h$adj.P.Val < 0.05])
  } else character(0)
  
  # Pull Chimp DEGs and wrap in unique()
  c_genes <- if (nm_C %in% names(ttbl_list)) {
    df_c <- ttbl_list[[nm_C]]
    df_c$Ensembl_ID <- as.character(df_c$Ensembl_ID)
    unique(df_c$Ensembl_ID[!is.na(df_c$adj.P.Val) & df_c$adj.P.Val < 0.05])
  } else character(0)
  ...
  plot_title <- paste0("DEGs H vs C - ", st)
  p <- plot(fit,
       fills = list(fill = c("#740949", "#A6AA59"), alpha = 0.6), # Dark Pink and Green
       labels = TRUE,
       quantities = TRUE, 
       main = plot_title
       )
...
for (stim in stimuli) {
  for (sp in c("H", "C")) {
    ...
    if (!(nm_2hr %in% names(toptable_list)) &
      ...
      message("Skipping ", stim, " (", sp, ") — no DEGs for either timepoint.")
      next
    }
    ...
    if (length(genes_2hr) == 0 & length(genes_24hr) == 0) {
      message("Skipping ", stim, " (", sp, ") — no DEGs for 2hr or 24hr.")
      next
    }
    ...
    # Fit and plot
    fit <- euler(venn_list, shape = "ellipse")
    plot_title <- paste0(stim, " (", sp, ") — 2hr vs 24hr DEGs")
...
plot_title <- paste0(stim, " — 2hr vs 24hr DEGs (", sp, ")")
p <- plot(
  fit,
...
for (st in stim_all) {
  ...
  df_h <- ttbl_list[[nm_H]] %>% 
    mutate(Ensembl_ID = as.character(Ensembl_ID))
  df_c <- ttbl_list[[nm_C]] %>% 
    mutate(Ensembl_ID = as.character(Ensembl_ID))
  
  # Merge by Entrez ID
  merged_df_all <- inner_join(df_h, df_c, by = "Ensembl_ID",
                              suffix = c("_H", "_C")) %>% 
    dplyr::select(Ensembl_ID, logFC_H = logFC_H, logFC_C = logFC_C)
